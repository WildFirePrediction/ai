"""
04 - Dataset Splitting (Per-episode environments)
Splits episode-based environments into train/val/test using stratification on
...
print("=" * 80)
print("DATASET SPLITTING (PER-EPISODE)")
print("=" * 80)

# ---------------------------------------------------------------------------
# 1. LOAD MANIFEST + EPISODE META
# ---------------------------------------------------------------------------
print("\n[1/4] Loading environment manifest...")
manifest = pd.read_parquet(tilling_dir / 'environments' / 'environment_manifest.parquet')
epi = pd.read_parquet(embedded_dir / 'episode_index_reclustered.parquet')
print(f"  Environments: {len(manifest)}")
print(f"  Episodes meta: {len(epi)}")

# Merge
envs = manifest.merge(epi[['episode_id','n_detections','duration_hours']], on='episode_id', how='left')
...
envs['size_cat'] = pd.cut(envs['n_detections'], bins=[0,20,100, np.inf], labels=['small','medium','large'])
