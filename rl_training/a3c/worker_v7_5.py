"""
A3C Worker V7.5 - Temporal LSTM with Memory Management

CRITICAL MEMORY OPTIMIZATIONS:
1. Aggressive tensor cleanup after each episode
2. Periodic garbage collection
3. Temporal environment with cache management
4. Same per-cell 8-neighbor prediction as V3
5. Dense rewards at every timestep
"""
import os
...
from a3c.model_v7_5 import A3C_TemporalLSTM
...
def worker_process_temporal(worker_id, shared_model, optimizer, filtered_episodes, config,
    ...
    """
    Worker process for A3C V7.5 with temporal LSTM and memory management.
    ...
    local_model = A3C_TemporalLSTM(
        ...
        max_burning_cells=config.get('max_burning_cells', 100)
    )
    local_model.train()
    ...
    # For periodic memory cleanup
    episodes_since_gc = 0
    gc_interval = 10  # Run garbage collection every 10 episodes
    ...
    while True:
        ...
        # Load temporal environment
        try:
            env = WildfireEnvTemporal(env_path, window_size=config['window_size'])
        ...
        # MEMORY OPTIMIZATION: Cap episode length
        max_episode_steps = config.get('max_episode_steps', 10)
        effective_max_length = min(max_length, max_episode_steps)

        while not done and steps < effective_max_length:
            # Convert obs_seq to tensor: (T, 14, H, W) → (1, T, 14, H, W)
            state_tensor = torch.from_numpy(obs_seq).unsqueeze(0).float()

            # Get current fire mask (from last timestep in sequence)
            # Fire mask is at channel 5
            fire_mask = state_tensor[0, -1, 5]  # (H, W) - last timestep, fire mask channel
            ...
            # Store trajectory
            states.append(state_tensor.detach())  # Detach to free computation graph
            actions.append(action_grid.detach())
            ...
            del state_tensor, fire_mask, action_grid, log_prob, entropy, value
        ...
        for t in range(len(states)):
            ...
            # Clear intermediate tensors
            del features, value_t, step_log_probs, step_entropies
        ...
        episodes_since_gc += 1
        if episodes_since_gc >= gc_interval:
            gc.collect()
            episodes_since_gc = 0
