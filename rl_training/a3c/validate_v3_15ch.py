"""
A3C V3 Validation Script for 15-Channel Models

Evaluates 15-channel V3 models (re-embedded data) on validation set.
NO channel conversion needed - direct 15ch inference.
"""
import os
import sys
...
def evaluate_episode(env_path, model, device, min_episode_length=4):
    """
    Evaluate a single episode with 15-channel model.
    
    Args:
        env_path: Path to environment pickle file
        model: Trained A3C model (15-channel)
        device: torch device
    ...
    with torch.no_grad():
        while not done:
            # Convert observation to tensor (15 channels)
            obs_tensor = torch.FloatTensor(obs).unsqueeze(0).to(device)  # (1, 15, H, W)
            
            # Extract fire mask (channel 5 after static features)
...
def validate_model(checkpoint_path, data_dir, split='val', min_episode_length=4, 
    ...
    """
    Run full validation on the 15-channel model.
    ...
    # Load model (15 channels)
    print(f"Loading 15-channel model from {checkpoint_path}")
    model = A3C_PerCellModel(in_channels=15)
    
    checkpoint = torch.load(checkpoint_path, map_location=device)
    if 'model_state_dict' in checkpoint:
        model.load_state_dict(checkpoint['model_state_dict'])
        if 'best_iou' in checkpoint:
            training_iou = checkpoint['best_iou']
            print(f"Training IoU: {training_iou:.4f} ({training_iou*100:.2f}%)")
    else:
    ...
    if use_wandb and WANDB_AVAILABLE:
        run = wandb.init(
            ...
            name=f"validation_15ch_{split}_{datetime.now().strftime('%y%m%d-%H%M')}",
            config={
    ...
    # Print results
    print("\n" + "="*80)
    print(f"VALIDATION RESULTS - {split.upper()} SET (15-CHANNEL MODEL)")
    print("="*80)
    ...
    if use_wandb and WANDB_AVAILABLE:
        ...
        plt.title(f'Distribution of Mean IoU - {split.upper()} Set (15ch)')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Validate A3C V3 15-channel model')
