"""
A3C Model V7 - Temporal 3D Conv for Wildfire Spread Prediction

Adds temporal context to V2 baseline (40.91% IoU):
- CNN encoder processes each timestep independently
- 3D Conv captures temporal dynamics (MUCH lighter than LSTM!)
...
class A3C_TemporalModel(nn.Module):
    """
    A3C model with temporal 3D Conv for per-cell 8-neighbor fire spread prediction.

    Architecture:
    1. CNN Encoder (per timestep): (14, H, W) → (64, H, W)
    2. Temporal 3D Conv: Process sequence of 3 timesteps → (128, H, W)
    3. Policy Head: Extract 3x3 features, predict 8 neighbors
    4. Value Head: Global average pooling → scalar value
    """

    def __init__(self, in_channels=14, window_size=3, lstm_hidden_dim=128, lstm_num_layers=2):
        super().__init__()

        self.window_size = window_size
        self.lstm_hidden_dim = lstm_hidden_dim  # Keep for compatibility
        self.lstm_num_layers = lstm_num_layers  # Keep for compatibility
        ...
        # Input: (B, 64, T, H, W) -> Output: (B, 128, H, W)
        self.temporal_conv = nn.Sequential(
            nn.Conv3d(64, 96, kernel_size=(3, 3, 3), padding=(0, 1, 1)),  # (B, 96, 1, H, W)
            nn.ReLU(),
            nn.Conv3d(96, 128, kernel_size=(1, 3, 3), padding=(0, 1, 1)),  # (B, 128, 1, H, W)
            nn.ReLU(),
        )
    ...
    def forward(self, x_seq, fire_mask):
        ...
        Returns:
            temporal_features: (B, 128, H, W) temporal-encoded features
            value: (B, 1) state value
        """
        B, T, C, H, W = x_seq.shape
        assert T == self.window_size, f"Expected {self.window_size} timesteps, got {T}"

        # Step 1: Encode each timestep with CNN
        feature_seq = []
        for t in range(T):
            obs_t = x_seq[:, t, :, :, :]  # (B, 14, H, W)
            feat_t = self.encoder(obs_t)  # (B, 64, H, W)
            feature_seq.append(feat_t)

        # Stack: (B, T, 64, H, W) -> (B, 64, T, H, W) for 3D conv
        feature_seq = torch.stack(feature_seq, dim=1)  # (B, T, 64, H, W)
        feature_seq = feature_seq.permute(0, 2, 1, 3, 4)  # (B, 64, T, H, W)

        # Step 2: Process temporal sequence with 3D Conv (MUCH LIGHTER!)
        temporal_features = self.temporal_conv(feature_seq)  # (B, 128, 1, H, W)
        temporal_features = temporal_features.squeeze(2)  # (B, 128, H, W)

        # Step 3: Compute global value
    ...
    def get_action_and_value(self, x_seq, fire_mask, action=None):
        ...
        Args:
            x_seq: (1, T, 14, H, W) sequence of environmental features (batch size must be 1)
