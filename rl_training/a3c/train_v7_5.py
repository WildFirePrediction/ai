"""
A3C Training V7.5 - Temporal LSTM (CORRECT Implementation)

CRITICAL IMPROVEMENTS FROM V7:
1. Keep FULL V3 encoder (128 channels) - NOT reduced!
2. Use LSTM for temporal state - NOT 3D conv
3. Memory-safe: 1-layer LSTM, 96 hidden dim (428K params)
4. Start with 2 workers to prevent memory issues
5. Window size = 3 timesteps
6. Same dense rewards and per-cell 8-neighbor prediction as V3
...
from a3c.model_v7_5 import A3C_TemporalLSTM, count_parameters
from a3c.worker_v7_5 import worker_process_temporal
from wildfire_env_temporal_v2 import WildfireEnvTemporal


def create_filtered_episode_list(env_paths, max_file_size_mb=50, min_episode_length=4):
    """
...
def main():
    parser = argparse.ArgumentParser(description='A3C V7.5 Training - Temporal LSTM')
    parser.add_argument('--repo-root', type=str, default='/home/chaseungjoon/code/WildfirePrediction')
    parser.add_argument('--num-workers', type=int, default=2, help='Number of parallel CPU workers (default: 2 for memory safety)')
    parser.add_argument('--max-episodes', type=int, default=1000, help='Total episodes across all workers')
    parser.add_argument('--lr', type=float, default=7e-5, help='Learning rate (V3 best: 7e-5)')
    parser.add_argument('--gamma', type=float, default=0.99, help='Discount factor')
    parser.add_argument('--value-loss-coef', type=float, default=0.5, help='Value loss coefficient')
    parser.add_argument('--entropy-coef', type=float, default=0.015, help='Entropy coefficient (V3 best: 0.015)')
    ...
    parser.add_argument('--min-episode-length', type=int, default=4, help='Min timesteps with burns per episode (V3 best: 4)')
    parser.add_argument('--window-size', type=int, default=3, help='Temporal window size (default: 3)')
    parser.add_argument('--lstm-hidden-dim', type=int, default=48, help='LSTM hidden dimension (default: 48 for memory)')
    parser.add_argument('--lstm-num-layers', type=int, default=1, help='Number of LSTM layers (default: 1 for memory)')
    parser.add_argument('--downsample-factor', type=int, default=2, help='Spatial downsampling before LSTM (default: 2 = 4x memory reduction)')
    parser.add_argument('--max-burning-cells', type=int, default=100, help='Max burning cells to process per step (default: 100)')
    parser.add_argument('--max-episode-steps', type=int, default=10, help='Max steps per episode (default: 10 for memory)')
    parser.add_argument('--use-gradient-checkpointing', action='store_true', help='Use gradient checkpointing (saves memory)')
    ...
    print(f"Downsample factor: {args.downsample_factor} (spatial size: {100//args.downsample_factor}x{100//args.downsample_factor})")
    print(f"Max burning cells: {args.max_burning_cells}")
    ...
    if use_wandb:
        wandb_run_name = args.wandb_run_name or f"a3c-v7.5-w{args.num_workers}-lstm{args.lstm_hidden_dim}h{args.lstm_num_layers}"
        wandb.init(
            project=args.wandb_project,
            name=wandb_run_name,
            config={
                "model": "A3C-V7.5-Temporal-LSTM",
                "formulation": "temporal_lstm_per_cell_8_neighbor",
                ...
                "max_burning_cells": args.max_burning_cells,
    ...
    # Create shared model on CPU (NOT GPU - A3C is CPU-based)
    shared_model = A3C_TemporalLSTM(
        in_channels=14,
        window_size=args.window_size,
        lstm_hidden_dim=args.lstm_hidden_dim,
        lstm_num_layers=args.lstm_num_layers,
    ...
    # Count parameters
    total_params = count_parameters(shared_model)
    print(f"Model parameters: {total_params:,}")
    print(f"V3 had 417K params, V7.5 has {total_params/1000:.1f}K params")

    # Create timestamped checkpoint directory
    from datetime import datetime
    timestamp = datetime.now().strftime("%y%m%d-%H%M")
    ckpt_dir = repo_root / 'rl_training' / 'a3c' / 'checkpoints_v7_5' / timestamp
    ...
    print(f"\n{'=' * 80}")
    print(f"Training started with {args.num_workers} workers!")
    print(f"{'=' * 80}\n")
    ...
    if use_wandb:
        artifact = wandb.Artifact(
            name='final-model-v7-5',
            type='model',
            description=f'A3C V7.5 (Temporal LSTM) after {global_episode_counter.value} episodes, IoU {global_best_iou.value:.4f}'
        )
