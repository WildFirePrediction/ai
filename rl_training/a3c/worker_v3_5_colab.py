"""
A3C Worker V3.5 - COLAB VERSION - Temporal Per-Pixel LSTM (FULL POWER)

Optimized for Google Colab with HIGH RAM + GPU:
1. Workers run on CPU for environment rollouts (4-8 parallel)
2. Gradients sent to GPU shared model for fast updates
3. Full 5 timestep temporal window (architecture plan)
4. Aggressive GPU utilization with larger batches
5. Memory-optimized but NOT artificially limited like local version

Colab advantages:
- 25GB+ RAM (vs 64GB local but with better allocation)
- V100/A100 GPU (16GB VRAM)
- Can handle 4-8 workers simultaneously
- No PyCharm or other memory hogs
"""
import os
...
def worker_process_temporal_colab(worker_id, shared_model, optimizer, filtered_episodes, config,
                                   global_episode_counter, global_best_iou, lock, metrics_queue=None):
    """
    Worker process for A3C V3.5 COLAB with temporal per-pixel LSTM.

    COLAB FULL POWER:
    - Local model runs on CPU for environment rollouts (parallel workers)
    - Gradients computed on CPU
    - Shared model on GPU receives gradients and performs fast updates
    - Full 5 timestep window (architecture plan spec)
    - 4-8 workers can run simultaneously on Colab

    Memory safety (Colab has 25GB+ RAM):
    ...
    - But NOT artificially limited like local version
    """
    ...
    temporal_window = config.get('temporal_window', 5)  # FULL 5 on Colab!
    local_model = A3C_TemporalModel(in_channels=14, temporal_window=temporal_window)
    local_model.train()
    # Keep local model on CPU - it will sync from shared model (which is on GPU)
    ...
    print(f"[Worker {worker_id}] Temporal window: {temporal_window} timesteps (FULL per-pixel LSTM)", flush=True)
    print(f"[Worker {worker_id}] Device: CPU (rollouts), gradients sent to GPU shared model", flush=True)
    ...
    while True:
        ...
        # Shared model is on GPU, so we copy to CPU for local rollouts
        with torch.no_grad():
        ...
        # Log grid size for monitoring (Colab can handle larger!)
        grid_cells = env.H * env.W
        if episode_count % 10 == 0 or grid_cells > 100000:
        ...
        # Store trajectory with DETACHED tensors only
        ...
        while not done and steps < max_length:
            # Don't convert to tensor yet - keep as numpy
            ...
            # Store numpy arrays and detached scalars
        ...
        # Delete environment immediately
        del env
        gc.collect()
        ...
        # Recompute values and log_probs WITH gradients
        ...
        for t in range(len(states)):
            ...
            # AGGRESSIVE CLEANUP: Delete ALL tensors from this timestep
        ...
        # Shared model is on GPU, so we need to move gradients appropriately
        with lock:
        ...
        # AGGRESSIVE cleanup of all tensors
        del states, actions, rewards, log_probs, values, entropies
        del returns_tensor, log_probs_tensor, values_tensor, entropy_tensor
        del advantages, policy_loss, value_loss, entropy_loss, total_loss
        del recomputed_log_probs, recomputed_values, recomputed_entropies
        ...
        # Every 5 episodes, clear torch cache (GPU on Colab)
