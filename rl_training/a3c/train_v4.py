"""
A3C Training V4 - Medium Capacity Architecture

Per-cell 8-neighbor prediction with dense rewards at every timestep.
Uses model_v3_medium: 3 layers with increased channels + GroupNorm.
Balanced: 2.24x params vs V2 (CPU-friendly for multi-worker).
"""
import os
...
from a3c.model_v3_medium import A3C_PerCellModel_Medium
from a3c.worker_v4 import worker_process_correct
...
def main():
    parser = argparse.ArgumentParser(description='A3C V4 Training - Medium Capacity Architecture')
    ...
    if use_wandb:
        wandb_run_name = args.wandb_run_name or f"a3c-v4-medium-w{args.num_workers}-lr{args.lr}"
        wandb.init(
            project=args.wandb_project,
            name=wandb_run_name,
            config={
                "model": "A3C-V4-Medium-3Layer",
                "architecture": "3-layer encoder (48→96→192) with GroupNorm",
                "model_params": "935K (2.24x increase from V2)",
    ...
    ckpt_dir = repo_root / 'rl_training' / 'a3c' / 'checkpoints_v4' / timestamp
