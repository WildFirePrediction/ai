"""
A3C Training V6 - Phase 1 Improvements

Improvements over V3:
1. Data augmentation (rotation + flipping) - 4-8x effective dataset size
2. Experience replay buffer - prevents catastrophic forgetting
3. 30% replay sampling - balance exploration/exploitation

Target: 45-50% IoU (from V3's 40.91%)
"""
import os
...
from a3c.worker_v6 import worker_process_v6
from a3c.replay_buffer import EpisodeReplayBuffer
...
def main():
    parser = argparse.ArgumentParser(description='A3C V6 Training - Data Augmentation + Experience Replay')
    ...
    parser.add_argument('--replay-capacity', type=int, default=100, help='Replay buffer capacity')
    parser.add_argument('--replay-prob', type=float, default=0.3, help='Probability of sampling from replay buffer')
    parser.add_argument('--replay-min-iou', type=float, default=0.15, help='Minimum IoU to add to replay buffer')
    ...
    print(f"=" * 80)
    print(f"A3C V6 Training - Phase 1 Improvements")
    print(f"=" * 80)
    print(f"Architecture: Per-cell 8-neighbor prediction (417K params)")
    print(f"Improvements:")
    print(f"  - Data augmentation: {'ENABLED' if not args.no_augmentation else 'DISABLED'}")
    print(f"  - Experience replay: {args.replay_capacity} episodes, {args.replay_prob:.1%} sampling")
    print(f"  - Min replay IoU: {args.replay_min_iou:.2f}")
    print(f"=" * 80)
    ...
    print(f"Effective dataset size with augmentation: {len(filtered_episodes) * 8} (8x augmentation)")
    print(f"{'=' * 80}\n")

    # Initialize WandB
    use_wandb = not args.no_wandb
    if use_wandb:
        wandb_run_name = args.wandb_run_name or f"a3c-v6-aug-replay-w{args.num_workers}-lr{args.lr}"
        wandb.init(
            project=args.wandb_project,
            name=wandb_run_name,
            config={
                "model": "A3C-V6-PerCell-8Neighbor-AugReplay",
                "version": "v6",
                "improvements": "data_augmentation+experience_replay",
                ...
                "replay_min_iou": args.replay_min_iou,
    ...
    shared_model = A3C_PerCellModel_V6(in_channels=14)
    ...
    ckpt_dir = repo_root / 'rl_training' / 'a3c' / 'checkpoints_v6' / timestamp
    ...
    # Create experience replay buffer (NEW in V6)
    replay_buffer = EpisodeReplayBuffer(
        capacity=args.replay_capacity,
        min_iou_threshold=args.replay_min_iou
    )
    print(f"Experience replay buffer initialized: capacity={args.replay_capacity}, min_iou={args.replay_min_iou}")
    ...
    for worker_id in range(args.num_workers):
        p = mp.Process(
            target=worker_process_v6,
            args=(worker_id, shared_model, optimizer, filtered_episodes, config,
                  global_episode_counter, global_best_iou, lock, replay_buffer, metrics_queue)
        )
        p.start()
        processes.append(p)
    ...
    print(f"Replay buffer stats:")
    print(f"  - Size: {replay_stats['size']}/{replay_stats['capacity']}")
    print(f"  - Mean IoU: {replay_stats['mean_iou']:.4f}")
    print(f"  - Max IoU: {replay_stats['max_iou']:.4f}")
    print(f"  - Total added: {replay_stats['total_added']}")
    ...
    if use_wandb:
        artifact = wandb.Artifact(
            name='final-model-v6-aug-replay',
            type='model',
            description=f'A3C V6 (augmentation+replay) after {global_episode_counter.value} episodes, IoU {global_best_iou.value:.4f}'
        )
        ...
        wandb.summary['replay_buffer_size'] = replay_stats['size']
        wandb.summary['replay_buffer_mean_iou'] = replay_stats['mean_iou']
