"""
A3C Model V6 - V3 Architecture with Data Augmentation + Experience Replay

Same architecture as V3 (417K params, 8-neighbor prediction), but trained with:
1. Data augmentation (rotation + flipping)
2. Experience replay buffer (top 100 episodes)
3. Improved training stability

This is Phase 1 of the improvement roadmap targeting 45-50% IoU.
"""
import torch
...
class A3C_PerCellModel_V6(nn.Module):
    """
    A3C model for per-cell 8-neighbor fire spread prediction (V6).
    ...
    - Per-cell policy head (local 3x3 features → 8-neighbor predictions)
    - Global value head (average pooling → scalar value)
    - 417K total parameters
    """

    def __init__(self, in_channels=14):
        super().__init__()

        # Shared CNN encoder (3 layers)
...
# For backward compatibility with V3 training code
A3C_PerCellModel = A3C_PerCellModel_V6
