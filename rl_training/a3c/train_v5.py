"""
A3C Training V5 - 4-Neighbor Multi-Task Learning
...
3. Better metrics: IoU, F1, precision, recall, accuracy
"""
import os
...
from a3c.worker_v5 import worker_process_multitask
...
def main():
    parser = argparse.ArgumentParser(description='A3C V5 Training - 4-Neighbor Multi-Task')
    ...
    print(f"=" * 80)
    print(f"A3C V5 Training - 4-Neighbor Multi-Task Learning")
    print(f"=" * 80)
    print(f"Problem: Per-cell 4-neighbor prediction (N, E, S, W)")
    print(f"Multi-task: Burn + Intensity + Temperature prediction")
    print(f"Metrics: IoU, F1, Precision, Recall, Accuracy")
    ...
    # Create shared model on CPU (V5: 4-neighbor multi-task)
    shared_model = A3C_PerCellModel_4Neighbor(
        in_channels=14,
        hidden_dim=128,
        use_groupnorm=True
    )
    ...
    print(f"Architecture: 4-neighbor prediction + multi-task learning")

    # Create timestamped checkpoint directory
    from datetime import datetime
    timestamp = datetime.now().strftime("%y%m%d-%H%M")
    ckpt_dir = repo_root / 'rl_training' / 'a3c' / 'checkpoints_v5' / timestamp
    ...
    # Shared counters and values (using F1 as primary metric now)
    global_episode_counter = mp.Value('i', 0)
    global_best_f1 = mp.Value('d', 0.0)
    lock = mp.Lock()
    ...
    config = {
        ...
        'hidden_dim': 128,  # For model initialization
    }

    # Create workers (V5: multi-task learning)
    processes = []
    for worker_id in range(args.num_workers):
        p = mp.Process(
            target=worker_process_multitask,
            args=(worker_id, shared_model, optimizer, filtered_episodes, config,
                  global_episode_counter, global_best_f1, lock, metrics_queue)
        )
        p.start()
        processes.append(p)
    ...
    if use_wandb:
        artifact = wandb.Artifact(
            name='final-model-v5-multitask',
            type='model',
            description=f'A3C V5 (4-neighbor multi-task) after {global_episode_counter.value} episodes, F1 {global_best_f1.value:.4f}'
        )
        artifact.add_file(str(final_path))
        wandb.log_artifact(artifact)

        wandb.summary['total_episodes'] = global_episode_counter.value
        wandb.summary['best_f1'] = global_best_f1.value
