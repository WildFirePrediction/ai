"""
A3C Worker V3 LSTM REL - 16 Channels with RELAXED IoU (8-neighbor tolerance)
Same architecture as V3_LSTM but with dilated ground truth for reward computation

Key Change:
- Uses 3x3 dilation on ground truth (8-neighbor tolerance)
- Matches U-Net V3 training approach for fair comparison
- More realistic reward signal for fire spread uncertainty
"""
import os
...
def worker_process_lstm(worker_id, shared_model, optimizer, filtered_episodes, config,
    ...
    """
    Worker process for A3C V3 LSTM with RELAXED IoU (8-neighbor tolerance).
    ...
    print(f"[Worker {worker_id}] Starting with {len(filtered_episodes)} episodes (sequence_length={sequence_length}, RELAXED IoU)", flush=True)
    ...
    while True:
        ...
        for t in range(start_t, min(start_t + max_length, T - 1)):
            ...
            # Compute reward with RELAXED IoU (8-neighbor tolerance)
