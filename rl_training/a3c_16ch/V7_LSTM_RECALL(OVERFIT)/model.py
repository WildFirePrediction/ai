"""
A3C Model V7 LARGE with LSTM - 16 Channels (SCALED UP)
Larger model for improved wildfire spread prediction

Improvements over V6:
- LSTM hidden: 512 (was 256) - 2x capacity
- Deeper CNN: 4 conv layers (was 3)
- Wider channels: 64→128→256→512 (was 64→128→256)
- Batch normalization (was GroupNorm)
- Larger policy head: 512 hidden (was 256)
- ~6M parameters (was ~1.8M) - 3.4x larger
...
class A3C_PerCellModel_LSTM(nn.Module):
    """
    LARGE A3C model with CNN encoder + LSTM for temporal context.

    Architecture:
    1. CNN Encoder: 4-layer deep (16ch -> 512 features) with BatchNorm
    2. LSTM: 512 hidden units for rich temporal modeling
    3. Policy/Value Heads: Larger capacity for better predictions
    """

    def __init__(self, in_channels=16, lstm_hidden=512, sequence_length=3):
        super().__init__()
        ...
        # CNN Encoder (per-timestep spatial processing) - DEEPER & WIDER
        # Input: (batch, 16, 30, 30) -> Output: (batch, 512, 30, 30)
        ...
        # Layer 4: 256 -> 512 (NEW - deeper)
        self.conv4 = nn.Conv2d(256, 512, kernel_size=3, padding=1)
        self.bn4 = nn.BatchNorm2d(512)
        self.dropout4 = nn.Dropout2d(0.1)

        # Spatial pooling to get per-timestep feature vector
        # (batch, 512, 30, 30) -> (batch, 512)
        self.spatial_pool = nn.AdaptiveAvgPool2d(1)

        # LSTM for temporal modeling - LARGER
        # Input: (sequence_length, batch, 512) -> Output: (sequence_length, batch, 512)
        self.lstm = nn.LSTM(
            input_size=512,
        ...
        # (batch, 512) -> (batch, 512, 30, 30)
        self.upsample = nn.Sequential(
            nn.Linear(lstm_hidden, 512 * 4),
            nn.ReLU(),
            nn.Unflatten(1, (512, 2, 2)),
            nn.Upsample(size=(30, 30), mode='bilinear', align_corners=False)
        )

        # Policy head - per-cell 8-neighbor prediction - LARGER
        self.policy_head = nn.Sequential(
            nn.Linear(512 * 9, 512),  # Was 256*9 -> 256
            nn.ReLU(),
            nn.Dropout(0.2),
    ...
    def encode_timestep(self, x):
        ...
        Returns:
            spatial_features: (batch, 512, 30, 30)
            pooled_features: (batch, 512)
        """
        # Layer 1
        ...
        # Layer 4 (NEW)
        spatial_features = F.relu(self.bn4(self.conv4(x)))
        spatial_features = self.dropout4(spatial_features)
    ...
    def forward(self, sequence, fire_mask):
        ...
        Returns:
            spatial_features: (batch, 512, 30, 30) - features for policy head
        ...
        for t in range(seq_len):
            _, pooled = self.encode_timestep(sequence[:, t])  # (batch, 512)
            pooled_sequence.append(pooled)

        # Stack into sequence: (seq_len, batch, 512)
        ...
        # Upsample to spatial features: (batch, 512) -> (batch, 512, 30, 30)
