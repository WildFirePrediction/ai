"""
A3C Model V4 with LSTM - 16 Channels (IMPROVED)
Improvements over V3:
1. Longer LSTM sequence (5 instead of 3)
2. Stronger numerical stability (1e-6 clamping)
3. Reward shaping for aggressive spread (in worker.py)
...
class A3C_PerCellModel_LSTM(nn.Module):
    ...
    V4 Improvements:
    - Longer sequence_length (5 vs 3)
    - Stronger probability clamping for numerical stability
    """

    def __init__(self, in_channels=16, lstm_hidden=256, sequence_length=5, use_groupnorm=True):
        super().__init__()
    ...
    def get_action_and_value(self, sequence, fire_mask, action=None):
        ...
        for cell_idx, (i, j) in enumerate(burning_cells):
            ...
            # V4 IMPROVEMENT: Stronger clamping for numerical stability
            probs_8d = torch.clamp(probs_8d, 1e-6, 1 - 1e-6)  # Was 1e-7 in V3
