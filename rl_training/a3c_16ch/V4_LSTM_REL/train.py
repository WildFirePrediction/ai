"""
A3C Training Script V4 LSTM REL - 16 Channels (IMPROVED)
Improvements over V3:
1. Longer LSTM sequence (5 instead of 3)
2. Stronger numerical stability (1e-6 clamping)
3. Reward shaping for aggressive spread
"""
import os
...
from a3c_16ch.V4_LSTM_REL.worker import worker_process_lstm
...
def main():
    parser = argparse.ArgumentParser(description='A3C V4 LSTM REL Training - Improved with Spread Bonus')
    ...
    parser.add_argument('--sequence-length', type=int, default=5,
                       help='LSTM sequence length (V4 default: 5)')
    ...
    print(f"A3C V4 LSTM REL Training - Improved with Spread Bonus")
    print(f"=" * 70)
    ...
    print(f"=" * 70)
    print(f"V4 Improvements:")
    print(f"  - LONGER LSTM (sequence_length={args.sequence_length}, was 3 in V3)")
    print(f"  - STRONGER STABILITY (1e-6 clamping, was 1e-7 in V3)")
    print(f"  - REWARD SHAPING (spread bonus for aggressive predictions)")
    print(f"=" * 70)
    print(f"Features:")
    print(f"  - LSTM temporal context (better momentum tracking)")
    ...
    print(f"  - Spread bonus: Encourages 10-50% cell growth per step")
    print(f"=" * 70)
    ...
    print(f"Reward shaping: Spread bonus (70% IoU + 30% spread)")
    print(f"=" * 70)
