"""
Validation Script for A3C V4 LSTM REL - 16 Channels (IMPROVED)
Evaluates trained model on validation set with 8-neighbor tolerance
V4 Improvements: Longer LSTM (5), Stronger stability, Reward shaping
"""
import os
...
def validate_episode(model, episode_file, sequence_length=5):
    """
    ...
    Args:
        ...
        sequence_length: LSTM sequence length (V4 default: 5)
...
def main():
    parser = argparse.ArgumentParser(description='A3C V4 LSTM REL Validation - Improved with Spread Bonus')
    ...
    print(f"A3C V4 LSTM REL Validation - Improved (Longer LSTM + Spread Bonus)")
    print(f"=" * 70)
    print(f"Checkpoint: {args.checkpoint}")
    print(f"Data directory: {args.data_dir}")
    print(f"Split: {args.split}")
    print(f"Sequence length: {args.sequence_length}")
    print(f"V4 Improvements: Longer LSTM (5), Stronger stability (1e-6), Spread bonus")
    ...
    print(f"\n" + "=" * 70)
    print(f"VALIDATION RESULTS (16ch LSTM V4 REL - RELAXED IoU)")
    print(f"=" * 70)
