"""
A3C Training Script V7 LSTM RECALL - LARGE MODEL with RECALL-FIRST SUPERAGGRO
16 Channels with Relaxed IoU + Recall-optimized reward system

LARGE MODEL IMPROVEMENTS:
- 3.4x larger model (~6M parameters vs ~1.8M)
- 512 LSTM hidden (was 256)
- Deeper/wider CNN with BatchNorm
- 12 workers (was 4) - 3x parallelism
- 25,000 episodes (was 10,000)
- Learning rate schedule (cosine annealing)
- Higher entropy coefficient (0.02 vs 0.01)
- Higher gradient clipping (1.0 vs 0.5)
...
from a3c_16ch.V7_LSTM_RECALL.worker import worker_process_lstm
...
class CosineAnnealingLR:
    ...
    def __init__(self, optimizer, initial_lr, min_lr, max_episodes):
    ...
    def step(self, episode):
        ...
        lr = self.min_lr + (self.initial_lr - self.min_lr) * \
             0.5 * (1 + math.cos(math.pi * episode / self.max_episodes))
...
def main():
    parser = argparse.ArgumentParser(description='A3C V7 LSTM RECALL (LARGE) Training - RECALL-FIRST SUPERAGGRO')
    ...
    parser.add_argument('--max-episodes', type=int, default=25000,
    ...
    print(f"A3C V7 LSTM RECALL (LARGE) Training - RECALL-FIRST SUPERAGGRO")
    print(f"=" * 70)
    ...
    print(f"=" * 70)
    print(f"LARGE MODEL IMPROVEMENTS:")
    print(f"  - ~6M parameters (3.4x larger than V6)")
    print(f"  - 512 LSTM hidden (was 256)")
    print(f"  - 4-layer CNN: 64→128→256→512 (was 3-layer)")
    print(f"  - BatchNorm (was GroupNorm)")
    print(f"  - Larger policy head: 512 units (was 256)")
    print(f"  - 12 workers (was 4)")
    print(f"  - 25k episodes (was 10k)")
    print(f"=" * 70)
    ...
    print(f"  - Higher entropy coef: 0.02 (more exploration)")
    print(f"  - Higher grad clip: 1.0 (larger updates)")
    print(f"=" * 70)
    ...
    # Create shared LARGE model
    shared_model = A3C_PerCellModel_LSTM(in_channels=16, lstm_hidden=512, sequence_length=args.sequence_length)
    ...
    print(f"  Size increase: {total_params / 1_777_353:.1f}x vs V6")

    # Create optimizer
    optimizer = torch.optim.Adam(shared_model.parameters(), lr=args.lr)

    # Create LR scheduler
    scheduler = CosineAnnealingLR(optimizer, args.lr, args.min_lr, args.max_episodes)
    ...
    config = {
        ...
        'value_loss_coef': 0.5,
        'entropy_coef': 0.02,  # HIGHER (was 0.01)
        'max_grad_norm': 1.0,  # HIGHER (was 0.5)
        'log_interval': 10,
        'sequence_length': args.sequence_length,
        'scheduler': scheduler,  # Pass scheduler to workers
    }
    ...
    print(f"\nStarting LARGE MODEL training with {args.num_workers} workers...")
    ...
    print(f"Learning rate schedule: {args.lr} → {args.min_lr} (cosine)")
    print(f"=" * 70)
    ...
    print(f"\n" + "=" * 70)
    print(f"LARGE MODEL Training completed!")
