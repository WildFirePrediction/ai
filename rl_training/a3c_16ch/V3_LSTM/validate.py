"""
Validation Script for A3C V3 LSTM - 16 Channels
Evaluates trained model on validation set with temporal sequences
"""
import os
...
def validate_episode(model, episode_file, sequence_length=3):
    ...
    Returns:
        mean_iou: Average IoU across all timesteps
    ...
    try:
        ...
        for t in range(sequence_length, T - 1):
            ...
            # Compute IoU
            iou = compute_iou(prediction, new_burns)
            ious.append(iou)
...
def main():
    parser = argparse.ArgumentParser(description='A3C V3 LSTM Validation - 16 Channels')
    ...
    print(f"A3C V3 LSTM Validation - 16 Channels (FULL)")
    print(f"=" * 70)
    ...
    print(f"Checkpoint best IoU: {checkpoint.get('best_iou', 'unknown'):.4f}")
    ...
    for i, episode_file in enumerate(episodes):
        ...
        if mean_iou is not None:
            ...
            if (i + 1) % 100 == 0 or (i + 1) == len(episodes):
                print(f"Progress: {i+1}/{len(episodes)} | Current mean IoU: {np.mean(ious):.4f}")
    ...
    print(f"\n" + "=" * 70)
    print(f"VALIDATION RESULTS (16ch LSTM FULL)")
    print(f"=" * 70)
    ...
    if args.output:
        ...
        output_path = checkpoint_dir / f'validation_results_{args.split}.npz'
