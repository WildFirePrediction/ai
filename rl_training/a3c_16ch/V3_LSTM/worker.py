"""
A3C Worker V3 LSTM - 16 Channels (FULL FEATURES)
Temporal context with LSTM + DATA AUGMENTATION + STRICT PENALTIES

Features:
- Temporal sequences for LSTM
- Data augmentation (rotation, flip)
- Strict penalty for non-movement when fire actually moves
- Per-cell 8-neighbor prediction with DENSE REWARDS
"""
import os
...
from a3c_16ch.V3_LSTM.model import A3C_PerCellModel_LSTM
...
def worker_process_lstm(worker_id, shared_model, optimizer, filtered_episodes, config,
    ...
    """
    Worker process for A3C V3 LSTM with temporal sequences, data augmentation, and strict penalties.
    ...
    print(f"[Worker {worker_id}] Starting with {len(filtered_episodes)} episodes (sequence_length={sequence_length})", flush=True)
    ...
    while True:
        ...
        for t in range(start_t, min(start_t + max_length, T - 1)):
            ...
            # Compute reward with STRICT PENALTIES for non-movement
            ...
            # STRICT PENALTY: If fire moved but model didn't predict anything
            fire_moved = new_burns.sum() > 0
            model_predicted = predicted_mask.sum() > 0

            if fire_moved and not model_predicted:
                # PENALTY: Fire moved but model was silent (-10 penalty)
                penalty = -10.0
                reward = penalty
            elif not fire_moved and model_predicted:
                # False alarm: predicted movement but fire didn't move (-5 penalty)
                penalty = -5.0
                reward = penalty
            else:
                # Normal IoU reward
                reward = step_iou * 10.0
