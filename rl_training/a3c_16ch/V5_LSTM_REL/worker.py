"""
A3C Worker V5 LSTM REL - AGGRESSIVE SAFETY-FIRST REWARD SHAPING with Data Augmentation
16 Channels with RELAXED IoU (8-neighbor tolerance) + Safety-biased reward system

AGGRESSIVE SAFETY-FIRST Reward System:
- Missing fire spread (false negative): -100.0 penalty (CATASTROPHIC - people in danger!)
- False alarm (false positive): +5.0 REWARD (Better safe than sorry - encourage caution!)
- Correct prediction: relaxed_iou * 50.0 (VERY HIGH REWARD for accuracy)
- Correct silence: +1.0 reward (small - don't encourage inaction)

Philosophy: It's better to warn about 10 areas that won't burn than to miss 1 area that will.
20:1 risk ratio (-100 vs +5) strongly encourages prediction when uncertain.
...
from a3c_16ch.V5_LSTM_REL.model import A3C_PerCellModel_LSTM
...
def worker_process_lstm(worker_id, shared_model, optimizer, filtered_episodes, config,
    ...
    """
    Worker process for A3C V5 LSTM with AGGRESSIVE SAFETY-FIRST reward shaping.

    AGGRESSIVE SAFETY-FIRST Reward System:
    - Missing fire spread (false negative): -100.0 penalty (CATASTROPHIC!)
    - False alarm (false positive): +5.0 REWARD (Better safe than sorry!)
    - Correct prediction: relaxed_iou * 50.0 (VERY HIGH REWARD)
    - Correct silence: +1.0 reward (small - don't encourage inaction)

    20:1 risk ratio (-100 vs +5) strongly biases toward prediction when uncertain.
    ...
    print(f"[Worker {worker_id}] Starting with {len(filtered_episodes)} episodes (sequence_length={sequence_length}, AGGRESSIVE SAFETY-FIRST)", flush=True)
    ...
    while True:
        ...
        for t in range(start_t, min(start_t + max_length, T - 1)):
            ...
            # AGGRESSIVE SAFETY-FIRST REWARD SHAPING
            fire_moved = new_burns.sum() > 0
            model_predicted = predicted_mask.sum() > 0

            if fire_moved and not model_predicted:
                # CATASTROPHIC FAILURE: Fire spread but model missed it
                # People could be in danger - MAXIMUM PENALTY
                reward = -100.0
                penalty_type = "MISS"
            elif not fire_moved and model_predicted:
                # False alarm: predicted movement but fire didn't move
                # REWARD THIS! Being cautious is GOOD for safety
                # Better to warn 10 areas unnecessarily than miss 1 real spread
                reward = +5.0
                penalty_type = "FALSE_ALARM_GOOD"
            else:
                # Model behavior aligns with reality
                if fire_moved and model_predicted:
                    # Both predicted and happened - reward based on accuracy
                    # VERY HIGH REWARD multiplier (50x) for accurate predictions
                    reward = step_iou * 50.0
                    penalty_type = "CORRECT_POSITIVE"
                else:
                    # Both didn't predict and didn't happen
                    # Small reward - don't encourage staying quiet
                    reward = 1.0
                    penalty_type = "CORRECT_NEGATIVE"
