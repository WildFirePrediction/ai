"""
A3C Training Script V5 LSTM REL - AGGRESSIVE SAFETY-FIRST Reward Shaping
16 Channels with Relaxed IoU + Safety-biased reward system
...
from a3c_16ch.V5_LSTM_REL.worker import worker_process_lstm
...
def main():
    parser = argparse.ArgumentParser(description='A3C V5 LSTM REL Training - AGGRESSIVE SAFETY-FIRST Reward Shaping')
    ...
    print(f"A3C V5 LSTM REL Training - AGGRESSIVE SAFETY-FIRST Reward Shaping")
    print(f"=" * 70)
    ...
    print(f"=" * 70)
    print(f"AGGRESSIVE SAFETY-FIRST Reward System:")
    print(f"  - Missing fire spread (false negative): -100.0 penalty (CATASTROPHIC!)")
    print(f"  - False alarm (false positive): +5.0 REWARD (Better safe than sorry!)")
    print(f"  - Correct prediction: relaxed_iou * 50.0 (VERY HIGH REWARD)")
    print(f"  - Correct silence: +1.0 reward (small)")
    print(f"=" * 70)
    print(f"Philosophy:")
    print(f"  Better to warn about 10 areas that won't burn than miss 1 that will.")
    print(f"  20:1 risk ratio (-100 vs +5) strongly biases toward prediction.")
    print(f"=" * 70)
    print(f"Features:")
    print(f"  - LSTM temporal context (sequence_length={args.sequence_length})")
    print(f"  - Data augmentation (rotation, flip)")
    print(f"  - AGGRESSIVE safety-first reward shaping")
    ...
    print(f"AGGRESSIVE safety-first reward: Enabled (rewards false alarms!)")
