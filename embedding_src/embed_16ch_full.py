"""
Embed fire episodes with 16 channels - FULL FEATURES:
- 2 DEM (slope, aspect)
- 9 Weather (temp, humidity, wind_speed, wind_dir, precip, pressure, cloud, visibility, dew_point)
- 1 NDVI (vegetation)
- 4 FSM (forest susceptibility, one-hot)

All weather channels with temporal variation + static terrain/vegetation features
"""
import numpy as np
import pandas as pd
import rasterio
...
def load_kma_weather(timestamp, stations):
    ...
    try:
        ...
        # AWS has: TA (temp), HM (humidity), WS1 (wind_speed), WD1 (wind_dir),
        # RN-15m (precip), PA (pressure), TD (dew_point)
        # Need to add: cloud_cover, visibility (not available, use zeros)
...
def embed_episode(episode_id, episode_df, dem_raster, ndvi_raster, fsm_raster,
    ...
    """
    Embed single fire episode with 16 channels
    ...
    for ts in timestamps:
        ...
        weather_grid = interpolate_weather_idw(weather_df, grid_x, grid_y)

        # Stack: 2 DEM + 9 Weather + 1 NDVI + 4 FSM = 16 channels
        features = np.concatenate([
            slope[np.newaxis, :, :],      # Ch 0
            aspect[np.newaxis, :, :],     # Ch 1
            weather_grid,                  # Ch 2-10: 9 weather channels
            ndvi[np.newaxis, :, :],       # Ch 11
            fsm_onehot                     # Ch 12-15: FSM classes 1-4
        ], axis=0)  # (16, 30, 30)
...
def main():
    print("="*80)
    print("EMBEDDING FIRE EPISODES - 16 CHANNELS (FULL FEATURES)")
    print("="*80)
    print("Channels:")
    ...
    output_dir = Path('embedded_data/fire_episodes_16ch_full')
